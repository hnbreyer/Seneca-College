
window.countriesData = [
  {
    code: 'AF',
    continent: 'Asia',
    areaInKm2: 652230,
    population: 35530081,
    capital: 'Kabul',
    name: {
      English: 'Afghanistan',
      Arabic: 'أفغانستان',
      Chinese: '阿富汗',
      French: 'Afghanistan',
      Hindi: 'अफ़ग़ानिस्तान',
      Korean: '아프가니스탄',
      Japanese: 'アフガニスタン',
      Russian: 'Афганистан'
    }
  },
  {
    code: 'AL',
    continent: 'Europe',
    areaInKm2: 28748,
    population: 2930187,
    capital: 'Tirana',
    name: {
      English: 'Albania',
      Arabic: 'ألبانيا',
      Chinese: '阿尔巴尼亚',
      French: 'Albanie',
      Hindi: 'अल्बानिया',
      Korean: '알바니아',
      Japanese: 'アルバニア',
      Russian: 'Албания'
    }
  },
  {
    code: 'DZ',
    continent: 'Africa',
    areaInKm2: 2381741,
    population: 41318142,
    capital: 'Algiers',
    name: {
      English: 'Algeria',
      Arabic: 'الجزائر',
      Chinese: '阿尔及利亚',
      French: 'Algérie',
      Hindi: 'एलजीरिया',
      Korean: '알제리',
      Japanese: 'アルジェリア',
      Russian: 'Алжир'
    }
  },
  {
    code: 'AD',
    continent: 'Europe',
    areaInKm2: 468,
    population: 76965,
    capital: 'Andorra la Vella',
    name: {
      English: 'Andorra',
      Arabic: 'أندورا',
      Chinese: '安道尔',
      French: 'Andorre',
      Hindi: 'अंडोरा',
      Korean: '안도라',
      Japanese: 'アンドラ',
      Russian: 'андорра'
    }
  },
  {
    code: 'AO',
    continent: 'Africa',
    areaInKm2: 1246700,
    population: 29784193,
    capital: 'Luanda',
    name: {
      English: 'Angola',
      Arabic: 'أنغولا',
      Chinese: '安哥拉',
      French: 'Angola',
      Hindi: 'अंगोला',
      Korean: '앙골라',
      Japanese: 'アンゴラ',
      Russian: 'Ангола'
    }
  },
  {
    code: 'AI',
    continent: 'Americas',
    areaInKm2: 91,
    population: 14909,
    capital: 'The Valley',
    name: {
      English: 'Anguilla',
      Arabic: 'أنغيلا',
      Chinese: '安圭拉',
      French: 'Anguilla',
      Hindi: 'एंगुइला',
      Korean: '앵 귈라',
      Japanese: 'アンギラ',
      Russian: 'Ангилья'
    }
  },
  {
    code: 'AQ',
    continent: 'Antarctica',
    areaInKm2: 14000000,
    population: '',
    capital: 'Antarctica',
    name: {
      English: 'Antarctica',
      Arabic: 'القارة القطبية الجنوبية',
      Chinese: '南极洲',
      French: 'Antarctique',
      Hindi: 'अंटार्कटिका',
      Korean: '남극 대륙',
      Japanese: '南極大陸',
      Russian: 'Антарктида'
    }
  },
  {
    code: 'AG',
    continent: 'Americas',
    areaInKm2: 442,
    population: 102012,
    capital: "St. John's",
    name: {
      English: 'Antigua and Barbuda',
      Arabic: 'أنتيغوا وبربودا',
      Chinese: '安提瓜和巴布达',
      French: 'Antigua-et-Barbuda',
      Hindi: 'अंतिगुया और बार्बूडा',
      Korean: '앤티가 바부 다',
      Japanese: 'アンティグアバーブーダ',
      Russian: 'Антигуа и Барбуда'
    }
  },
  {
    code: 'AR',
    continent: 'Americas',
    areaInKm2: 2780400,
    population: 44271041,
    capital: 'Buenos Aires',
    name: {
      English: 'Argentina',
      Arabic: 'الأرجنتين',
      Chinese: '阿根廷',
      French: 'Argentine',
      Hindi: 'अर्जेंटीना',
      Korean: '아르헨티나',
      Japanese: 'アルゼンチン',
      Russian: 'Аргентина'
    }
  },
  {
    code: 'AM',
    continent: 'Asia',
    areaInKm2: 29743,
    population: 2930450,
    capital: 'Yerevan',
    name: {
      English: 'Armenia',
      Arabic: 'أرمينيا',
      Chinese: '亚美尼亚',
      French: 'Arménie',
      Hindi: 'आर्मीनिया',
      Korean: '아르메니아',
      Japanese: 'アルメニア',
      Russian: 'Армения'
    }
  },
  {
    code: 'AW',
    continent: 'Americas',
    areaInKm2: 180,
    population: 105264,
    capital: 'Oranjestad',
    name: {
      English: 'Aruba',
      Arabic: 'أروبا',
      Chinese: '阿鲁巴',
      French: 'Aruba',
      Hindi: 'अरूबा',
      Korean: '아루바',
      Japanese: 'アルバ',
      Russian: 'Аруба'
    }
  },
  {
    code: 'AU',
    continent: 'Oceania',
    areaInKm2: 7692024,
    population: 24450561,
    capital: 'Canberra',
    name: {
      English: 'Australia',
      Arabic: 'أستراليا',
      Chinese: '澳大利亚',
      French: 'Australie',
      Hindi: 'ऑस्ट्रेलिया',
      Korean: '호주',
      Japanese: 'オーストラリア',
      Russian: 'Австралия'
    }
  },
  {
    code: 'AT',
    continent: 'Europe',
    areaInKm2: 83871,
    population: 8735453,
    capital: 'Vienna',
    name: {
      English: 'Austria',
      Arabic: 'النمسا',
      Chinese: '奥地利',
      French: "L'Autriche",
      Hindi: 'ऑस्ट्रिया',
      Korean: '오스트리아',
      Japanese: 'オーストリア',
      Russian: 'Австрия'
    }
  },
  {
    code: 'AZ',
    continent: 'Asia',
    areaInKm2: 86600,
    population: 9827589,
    capital: 'Baku',
    name: {
      English: 'Azerbaijan',
      Arabic: 'أذربيجان',
      Chinese: '阿塞拜疆',
      French: 'Azerbaïdjan',
      Hindi: 'आज़रबाइजान',
      Korean: '아제르바이잔',
      Japanese: 'アゼルバイジャン',
      Russian: 'Азербайджан'
    }
  },
  {
    code: 'BS',
    continent: 'Americas',
    areaInKm2: 13943,
    population: 395361,
    capital: 'Nassau',
    name: {
      English: 'Bahamas',
      Arabic: 'الباهاما',
      Chinese: '巴哈马',
      French: 'Bahamas',
      Hindi: 'बहामा',
      Korean: '바하마',
      Japanese: 'バハマ',
      Russian: 'Багамские о- ва'
    }
  },
  {
    code: 'BH',
    continent: 'Asia',
    areaInKm2: 765,
    population: 1492584,
    capital: 'Manama',
    name: {
      English: 'Bahrain',
      Arabic: 'البحرين',
      Chinese: '巴林',
      French: 'Bahreïn',
      Hindi: 'बहरीन',
      Korean: '바레인',
      Japanese: 'バーレーン',
      Russian: 'Бахрейн'
    }
  },
  {
    code: 'BD',
    continent: 'Asia',
    areaInKm2: 147570,
    population: 164669751,
    capital: 'Dhaka',
    name: {
      English: 'Bangladesh',
      Arabic: 'بنغلاديش',
      Chinese: '孟加拉国',
      French: 'Bangladesh',
      Hindi: 'बांग्लादेश',
      Korean: '방글라데시',
      Japanese: 'バングラデシュ',
      Russian: 'Бангладеш'
    }
  },
  {
    code: 'BB',
    continent: 'Americas',
    areaInKm2: 430,
    population: 285719,
    capital: 'Bridgetown',
    name: {
      English: 'Barbados',
      Arabic: 'بربادوس',
      Chinese: '巴巴多斯',
      French: 'La Barbade',
      Hindi: 'बारबाडोस',
      Korean: '바베이도스',
      Japanese: 'バルバドス',
      Russian: 'Барбадос'
    }
  },
  {
    code: 'BY',
    continent: 'Europe',
    areaInKm2: 207600,
    population: 9468338,
    capital: 'Minsk',
    name: {
      English: 'Belarus',
      Arabic: 'روسيا البيضاء',
      Chinese: '白俄罗斯',
      French: 'Bélarus',
      Hindi: 'बेलोरूस',
      Korean: '벨라루스',
      Japanese: 'ベラルーシ',
      Russian: 'Беларусь'
    }
  },
  {
    code: 'BE',
    continent: 'Europe',
    areaInKm2: 30528,
    population: 11429336,
    capital: 'Brussels',
    name: {
      English: 'Belgium',
      Arabic: 'بلجيكا',
      Chinese: '比利时',
      French: 'la Belgique',
      Hindi: 'बेल्जियम',
      Korean: '벨기에',
      Japanese: 'ベルギー',
      Russian: 'Бельгия'
    }
  },
  {
    code: 'BZ',
    continent: 'Americas',
    areaInKm2: 22966,
    population: 374681,
    capital: 'Belmopan',
    name: {
      English: 'Belize',
      Arabic: 'بليز',
      Chinese: '伯利兹',
      French: 'Belize',
      Hindi: 'बेलीज',
      Korean: '벨리즈',
      Japanese: 'ベリーズ',
      Russian: 'Белиз'
    }
  },
  {
    code: 'BJ',
    continent: 'Africa',
    areaInKm2: 114763,
    population: 11175692,
    capital: 'Porto Novo',
    name: {
      English: 'Benin',
      Arabic: 'بنين',
      Chinese: '贝宁',
      French: 'Bénin',
      Hindi: 'बेनिन',
      Korean: '베냉',
      Japanese: 'ベニン',
      Russian: 'Бенин'
    }
  },
  {
    code: 'BM',
    continent: 'Americas',
    areaInKm2: 54,
    population: 61349,
    capital: 'Hamilton',
    name: {
      English: 'Bermuda',
      Arabic: 'برمودا',
      Chinese: '百慕大',
      French: 'Bermudes',
      Hindi: 'बरमूडा',
      Korean: '버뮤다',
      Japanese: 'バミューダ',
      Russian: 'Бермудские острова'
    }
  },
  {
    code: 'BT',
    continent: 'Asia',
    areaInKm2: 38394,
    population: 807610,
    capital: 'Thimphu',
    name: {
      English: 'Bhutan',
      Arabic: 'بوتان',
      Chinese: '不丹',
      French: 'Bhoutan',
      Hindi: 'भूटान',
      Korean: '부탄',
      Japanese: 'ブータン',
      Russian: 'Бутан'
    }
  },
  {
    code: 'BO',
    continent: 'Americas',
    areaInKm2: 1098581,
    population: 11051600,
    capital: 'La Paz',
    name: {
      English: 'Bolivia (Plurinational State of)',
      Arabic: 'بوليفيا ',
      Chinese: '玻利维亚',
      French: 'Bolivie',
      Hindi: 'बोलीविया',
      Korean: '볼리비아',
      Japanese: 'ボリビア',
      Russian: 'Боливия'
    }
  },
  {
    code: 'BA',
    continent: 'Europe',
    areaInKm2: 51209,
    population: 3507017,
    capital: 'Sarajevo',
    name: {
      English: 'Bosnia and Herzegovina',
      Arabic: 'البوسنة والهرسك',
      Chinese: '波斯尼亚和黑塞哥维那',
      French: 'Bosnie Herzégovine',
      Hindi: 'बोस्निया और हर्जेगोविना',
      Korean: '보스니아 헤르체고비나',
      Japanese: 'ボスニア・ヘルツェゴビナ',
      Russian: 'Босния и Герцеговина'
    }
  },
  {
    code: 'BW',
    continent: 'Africa',
    areaInKm2: 581730,
    population: 2291661,
    capital: 'Gaborone',
    name: {
      English: 'Botswana',
      Arabic: 'بوتسوانا',
      Chinese: '博茨瓦纳',
      French: 'Botswana',
      Hindi: 'बोत्सवाना',
      Korean: '보츠와나',
      Japanese: 'ボツワナ',
      Russian: 'Ботсвана'
    }
  },
  {
    code: 'BR',
    continent: 'Americas',
    areaInKm2: 8515767,
    population: 209288278,
    capital: 'Brasília',
    name: {
      English: 'Brazil',
      Arabic: 'البرازيل',
      Chinese: '巴西',
      French: 'Brésil',
      Hindi: 'ब्राज़िल',
      Korean: '브라질',
      Japanese: 'ブラジル',
      Russian: 'Бразилия'
    }
  },
  {
    code: 'BN',
    continent: 'Asia',
    areaInKm2: 5765,
    population: 428697,
    capital: 'Bandar Seri Begawan',
    name: {
      English: 'Brunei Darussalam',
      Arabic: 'بروناي دار السلام',
      Chinese: '文莱达鲁萨兰国',
      French: 'Brunei Darussalam',
      Hindi: 'ब्रुनेई दारुस्सलाम',
      Korean: '브루나이 다루 살람',
      Japanese: 'ブルネイダルサラーム',
      Russian: 'Бруней-Даруссалам'
    }
  },
  {
    code: 'BG',
    continent: 'Europe',
    areaInKm2: 110879,
    population: 7084571,
    capital: 'Sofia',
    name: {
      English: 'Bulgaria',
      Arabic: 'بلغاريا',
      Chinese: '保加利亚',
      French: 'Bulgarie',
      Hindi: 'बुल्गारिया',
      Korean: '불가리아',
      Japanese: 'ブルガリア',
      Russian: 'Болгария'
    }
  },
  {
    code: 'BF',
    continent: 'Africa',
    areaInKm2: 274222,
    population: 19193382,
    capital: 'Ouagadougou',
    name: {
      English: 'Burkina Faso',
      Arabic: 'بوركينا فاسو',
      Chinese: '布基纳法索',
      French: 'Burkina Faso',
      Hindi: 'बुर्किना फासो',
      Korean: '부키 나 파소',
      Japanese: 'ブルキナファソ',
      Russian: 'Буркина-Фасо'
    }
  },
  {
    code: 'BI',
    continent: 'Africa',
    areaInKm2: 27834,
    population: 10864245,
    capital: 'Bujumbura',
    name: {
      English: 'Burundi',
      Arabic: 'بوروندي',
      Chinese: '布隆迪',
      French: 'Burundi',
      Hindi: 'बुस्र्न्दी',
      Korean: '부룬디',
      Japanese: 'ブルンジ',
      Russian: 'Бурунди'
    }
  },
  {
    code: 'CV',
    continent: 'Africa',
    areaInKm2: '',
    population: 546388,
    capital: 'Praia',
    name: {
      English: 'Cabo Verde',
      Arabic: 'كابو فيردي',
      Chinese: '佛得角',
      French: 'Cabo Verde',
      Hindi: 'काबो वर्डे',
      Korean: '카보 베르데',
      Japanese: 'カボベルデ',
      Russian: 'Кабо-Верде'
    }
  },
  {
    code: 'KH',
    continent: 'Asia',
    areaInKm2: 181035,
    population: 16005373,
    capital: 'Phnom Penh',
    name: {
      English: 'Cambodia',
      Arabic: 'كمبوديا',
      Chinese: '柬埔寨',
      French: 'Cambodge',
      Hindi: 'कंबोडिया',
      Korean: '캄보디아',
      Japanese: 'カンボジア',
      Russian: 'Камбоджа'
    }
  },
  {
    code: 'CM',
    continent: 'Africa',
    areaInKm2: 475442,
    population: 24053727,
    capital: 'Yaoundé',
    name: {
      English: 'Cameroon',
      Arabic: 'الكاميرون',
      Chinese: '喀麦隆',
      French: 'Cameroun',
      Hindi: 'कैमरून',
      Korean: '카메룬',
      Japanese: 'カメルーン',
      Russian: 'Камерун'
    }
  },
  {
    code: 'CA',
    continent: 'Americas',
    areaInKm2: 9984670,
    population: 36624199,
    capital: 'Ottawa',
    name: {
      English: 'Canada',
      Arabic: 'كندا',
      Chinese: '加拿大',
      French: 'Canada',
      Hindi: 'कनाडा',
      Korean: '캐나다',
      Japanese: 'カナダ',
      Russian: 'Канада'
    }
  },
  {
    code: 'KY',
    continent: 'Americas',
    areaInKm2: 264,
    population: 61559,
    capital: 'George Town',
    name: {
      English: 'Cayman Islands',
      Arabic: 'جزر كايمان',
      Chinese: '开曼群岛',
      French: 'Îles Caïmans',
      Hindi: 'केमैन टापू',
      Korean: '케이맨 제도',
      Japanese: 'ケイマン諸島',
      Russian: 'Каймановы острова'
    }
  },
  {
    code: 'CF',
    continent: 'Africa',
    areaInKm2: 622984,
    population: 4659080,
    capital: 'Bangui',
    name: {
      English: 'Central African Republic',
      Arabic: 'جمهورية افريقيا الوسطى',
      Chinese: '中非共和国',
      French: 'République centrafricaine',
      Hindi: 'केंद्रीय अफ्रीकन गणराज्य',
      Korean: '중앙 아프리카 공화국',
      Japanese: '中央アフリカ共和国',
      Russian: 'Центрально-Африканская Республика'
    }
  },
  {
    code: 'TD',
    continent: 'Africa',
    areaInKm2: 1284000,
    population: 14899994,
    capital: "N'Djamena",
    name: {
      English: 'Chad',
      Arabic: 'تشاد',
      Chinese: '乍得',
      French: 'Tchad',
      Hindi: 'काग़ज़ का टुकड़ा',
      Korean: '차드',
      Japanese: 'チャド',
      Russian: 'Чад'
    }
  },
  {
    code: 'CL',
    continent: 'Americas',
    areaInKm2: 756102,
    population: 18054726,
    capital: 'Santiago',
    name: {
      English: 'Chile',
      Arabic: 'تشيلي',
      Chinese: '智利',
      French: 'Chili',
      Hindi: 'चिली',
      Korean: '칠레',
      Japanese: 'チリ',
      Russian: 'Чили'
    }
  },
  {
    code: 'CN',
    continent: 'Asia',
    areaInKm2: 9640011,
    population: 1409517397,
    capital: 'Beijing',
    name: {
      English: 'China',
      Arabic: 'الصين',
      Chinese: '中国',
      French: 'Chine',
      Hindi: 'चीन',
      Korean: '중국',
      Japanese: '中国',
      Russian: 'Китай'
    }
  },
  {
    code: 'CX',
    continent: 'n/a',
    areaInKm2: 135,
    population: 'n/a',
    capital: 'Flying Fish Cove',
    name: {
      English: 'Christmas Island',
      Arabic: 'جزيرة الكريسماس',
      Chinese: '圣诞岛',
      French: "L'île de noël",
      Hindi: 'क्रिसमस द्वीप',
      Korean: '크리스마스 섬',
      Japanese: 'クリスマス島',
      Russian: 'Остров Рождества'
    }
  },
  {
    code: 'CC',
    continent: 'n/a',
    areaInKm2: 14,
    population: 'n/a',
    capital: 'West Island',
    name: {
      English: 'Cocos (Keeling) Islands',
      Arabic: 'جزر كوكوس (كيلينغ)',
      Chinese: '科科斯（基林）群岛',
      French: 'Îles Cocos (Keeling)',
      Hindi: 'कोकोस (कीलिंग) द्वीप समूह',
      Korean: '코코스 군도',
      Japanese: 'ココス諸島',
      Russian: 'Кокосовые (Килинг) острова'
    }
  },
  {
    code: 'CO',
    continent: 'Americas',
    areaInKm2: 1141748,
    population: 49065615,
    capital: 'Bogotá',
    name: {
      English: 'Colombia',
      Arabic: 'كولومبيا',
      Chinese: '哥伦比亚',
      French: 'Colombie',
      Hindi: 'कोलम्बिया',
      Korean: '콜롬비아',
      Japanese: 'コロンビア',
      Russian: 'Колумбия'
    }
  },
  {
    code: 'KM',
    continent: 'Africa',
    areaInKm2: 1862,
    population: 813912,
    capital: 'Moroni',
    name: {
      English: 'Comoros',
      Arabic: 'جزر القمر',
      Chinese: '科摩罗',
      French: 'Comores',
      Hindi: 'कोमोरोस',
      Korean: '코모로',
      Japanese: 'コモロ',
      Russian: 'Коморские острова'
    }
  },
  {
    code: 'CG',
    continent: 'Africa',
    areaInKm2: 2344858,
    population: 5260750,
    capital: 'Brazzaville',
    name: {
      English: 'Congo',
      Arabic: 'الكونغو',
      Chinese: '刚果',
      French: 'Congo',
      Hindi: 'कांगो',
      Korean: '콩고',
      Japanese: 'コンゴ',
      Russian: 'Конго'
    }
  },
  {
    code: 'CD',
    continent: 'Africa',
    areaInKm2: 342000,
    population: 81339988,
    capital: 'Kinshasa',
    name: {
      English: 'Congo (Democratic Republic of the)',
      Arabic: 'الكونغو (جمهورية - الديمقراطية)',
      Chinese: '刚果（民主共和国）',
      French: 'Congo (République démocratique du)',
      Hindi: 'कांगो (लोकतांत्रिक गणराज्य)',
      Korean: '콩고 민주 공화국',
      Japanese: 'コンゴ民主共和国',
      Russian: 'Конго (Демократическая Республика)'
    }
  },
  {
    code: 'CR',
    continent: 'Americas',
    areaInKm2: 51100,
    population: 4905769,
    capital: 'San José',
    name: {
      English: 'Costa Rica',
      Arabic: 'كوستا ريكا',
      Chinese: '哥斯达黎加',
      French: 'Costa Rica',
      Hindi: 'कोस्टा रिका',
      Korean: '코스타리카',
      Japanese: 'コスタリカ',
      Russian: 'Коста-Рика'
    }
  },
  {
    code: 'HR',
    continent: 'Europe',
    areaInKm2: 56594,
    population: 4189353,
    capital: 'Zagreb',
    name: {
      English: 'Croatia',
      Arabic: 'كرواتيا',
      Chinese: '克罗地亚',
      French: 'Croatie',
      Hindi: 'क्रोएशिया',
      Korean: '크로아티아',
      Japanese: 'クロアチア',
      Russian: 'Хорватия'
    }
  },
  {
    code: 'CU',
    continent: 'Americas',
    areaInKm2: 109884,
    population: 11484636,
    capital: 'Havana',
    name: {
      English: 'Cuba',
      Arabic: 'كوبا',
      Chinese: '古巴',
      French: 'Cuba',
      Hindi: 'क्यूबा',
      Korean: '쿠바',
      Japanese: 'キューバ',
      Russian: 'Куба'
    }
  },
  {
    code: 'CW',
    continent: 'Americas',
    areaInKm2: 444,
    population: 160539,
    capital: 'Willemstad',
    name: {
      English: 'Curaçao',
      Arabic: 'كوراساو',
      Chinese: '库拉索',
      French: 'Curacao',
      Hindi: 'कुराकाओ',
      Korean: '쿠라 사오',
      Japanese: 'キュラソー',
      Russian: 'Кюрасао'
    }
  },
  {
    code: 'CY',
    continent: 'Europe',
    areaInKm2: 9251,
    population: 1179551,
    capital: 'Nicosia',
    name: {
      English: 'Cyprus',
      Arabic: 'قبرص',
      Chinese: '塞浦路斯',
      French: 'Chypre',
      Hindi: 'साइप्रस',
      Korean: '키프로스',
      Japanese: 'キプロス',
      Russian: 'Кипр'
    }
  },
  {
    code: 'CZ',
    continent: 'Europe',
    areaInKm2: 78865,
    population: 10618303,
    capital: 'Prague',
    name: {
      English: 'Czechia',
      Arabic: 'التشيك',
      Chinese: '捷克',
      French: 'République tchèque',
      Hindi: 'Czechia',
      Korean: '체코',
      Japanese: 'チェコ',
      Russian: 'Чехия'
    }
  },
  {
    code: 'DK',
    continent: 'Europe',
    areaInKm2: 43094,
    population: 5733551,
    capital: 'Copenhagen',
    name: {
      English: 'Denmark',
      Arabic: 'الدنمارك',
      Chinese: '丹麦',
      French: 'Danemark',
      Hindi: 'डेनमार्क',
      Korean: '덴마크',
      Japanese: 'デンマーク',
      Russian: 'Дания'
    }
  },
  {
    code: 'DJ',
    continent: 'Africa',
    areaInKm2: 23200,
    population: 956985,
    capital: 'Djibouti',
    name: {
      English: 'Djibouti',
      Arabic: 'جيبوتي',
      Chinese: '吉布提',
      French: 'Djibouti',
      Hindi: 'जिबूती',
      Korean: '지부티',
      Japanese: 'ジブチ',
      Russian: 'Джибути'
    }
  },
  {
    code: 'DM',
    continent: 'Americas',
    areaInKm2: 751,
    population: 73925,
    capital: 'Roseau',
    name: {
      English: 'Dominica',
      Arabic: 'دومينيكا',
      Chinese: '多米尼加',
      French: 'Dominique',
      Hindi: 'डोमिनिका',
      Korean: '도미니카 공화국',
      Japanese: 'ドミニカ',
      Russian: 'Доминика'
    }
  },
  {
    code: 'DO',
    continent: 'Americas',
    areaInKm2: 48671,
    population: 10766998,
    capital: 'Santo Domingo',
    name: {
      English: 'Dominican Republic',
      Arabic: 'جمهورية الدومنيكان',
      Chinese: '多明尼加共和国',
      French: 'République Dominicaine',
      Hindi: 'डोमिनिकन गणराज्य',
      Korean: '도미니카 공화국',
      Japanese: 'ドミニカ共和国',
      Russian: 'Доминиканская Республика'
    }
  },
  {
    code: 'EC',
    continent: 'Americas',
    areaInKm2: 276841,
    population: 16624858,
    capital: 'Quito',
    name: {
      English: 'Ecuador',
      Arabic: 'الإكوادور',
      Chinese: '厄瓜多尔',
      French: 'Équateur',
      Hindi: 'इक्वेडोर',
      Korean: '에콰도르',
      Japanese: 'エクアドル',
      Russian: 'Эквадор'
    }
  },
  {
    code: 'EG',
    continent: 'Africa',
    areaInKm2: 1002450,
    population: 97553151,
    capital: 'Cairo',
    name: {
      English: 'Egypt',
      Arabic: 'مصر',
      Chinese: '埃及',
      French: 'Egypte',
      Hindi: 'मिस्र',
      Korean: '이집트',
      Japanese: 'エジプト',
      Russian: 'Египет'
    }
  },
  {
    code: 'SV',
    continent: 'Americas',
    areaInKm2: 21041,
    population: 6377853,
    capital: 'San Salvador',
    name: {
      English: 'El Salvador',
      Arabic: 'السلفادور',
      Chinese: '萨尔瓦多',
      French: 'Le Salvador',
      Hindi: 'एल साल्वाडोर',
      Korean: '엘살바도르',
      Japanese: 'エルサルバドル',
      Russian: 'Сальвадор'
    }
  },
  {
    code: 'GQ',
    continent: 'Africa',
    areaInKm2: 28051,
    population: 1267689,
    capital: 'Malabo',
    name: {
      English: 'Equatorial Guinea',
      Arabic: 'غينيا الإستوائية',
      Chinese: '赤道几内亚',
      French: 'Guinée Équatoriale',
      Hindi: 'भूमध्यवर्ती गिनी',
      Korean: '적도 기니',
      Japanese: '赤道ギニア',
      Russian: 'Экваториальная Гвинея'
    }
  },
  {
    code: 'ER',
    continent: 'Africa',
    areaInKm2: 117600,
    population: 5068831,
    capital: 'Asmara',
    name: {
      English: 'Eritrea',
      Arabic: 'إريتريا',
      Chinese: '厄立特里亚',
      French: 'Érythrée',
      Hindi: 'इरिट्रिया',
      Korean: '에리트레아',
      Japanese: 'エリトリア',
      Russian: 'Эритрея'
    }
  },
  {
    code: 'EE',
    continent: 'Europe',
    areaInKm2: 45227,
    population: 1309632,
    capital: 'Tallinn',
    name: {
      English: 'Estonia',
      Arabic: 'استونيا',
      Chinese: '爱沙尼亚',
      French: 'Estonie',
      Hindi: 'एस्तोनिया',
      Korean: '에스토니아',
      Japanese: 'エストニア',
      Russian: 'Эстония'
    }
  },
  {
    code: 'ET',
    continent: 'Africa',
    areaInKm2: 1104300,
    population: 104957438,
    capital: 'Addis Ababa',
    name: {
      English: 'Ethiopia',
      Arabic: 'أثيوبيا',
      Chinese: '埃塞俄比亚',
      French: 'Ethiopie',
      Hindi: 'इथियोपिया',
      Korean: '에티오피아',
      Japanese: 'エチオピア',
      Russian: 'Эфиопия'
    }
  },
  {
    code: 'FJ',
    continent: 'Oceania',
    areaInKm2: 18272,
    population: 905502,
    capital: 'Suva',
    name: {
      English: 'Fiji',
      Arabic: 'فيجي',
      Chinese: '斐济',
      French: 'Fidji',
      Hindi: 'फ़िजी',
      Korean: '피지',
      Japanese: 'フィジー',
      Russian: 'Фиджи'
    }
  },
  {
    code: 'FI',
    continent: 'Europe',
    areaInKm2: 338424,
    population: 5523231,
    capital: 'Helsinki',
    name: {
      English: 'Finland',
      Arabic: 'فنلندا',
      Chinese: '芬兰',
      French: 'Finlande',
      Hindi: 'फिनलैंड',
      Korean: '핀란드',
      Japanese: 'フィンランド',
      Russian: 'Финляндия'
    }
  },
  {
    code: 'FR',
    continent: 'Europe',
    areaInKm2: 640679,
    population: 64979548,
    capital: 'Paris',
    name: {
      English: 'France',
      Arabic: 'فرنسا',
      Chinese: '法国',
      French: 'France',
      Hindi: 'फ्रांस',
      Korean: '프랑스',
      Japanese: 'フランス',
      Russian: 'Франция'
    }
  },
  {
    code: 'GF',
    continent: 'Americas',
    areaInKm2: 4167,
    population: 282731,
    capital: 'Cayenne',
    name: {
      English: 'French Guiana',
      Arabic: 'غيانا الفرنسية',
      Chinese: '法属圭亚那',
      French: 'Guinée Française',
      Hindi: 'फ्रेंच गयाना',
      Korean: '프랑스 령 기아나',
      Japanese: 'フランス領ギアナ',
      Russian: 'Французская Гвиана'
    }
  },
  {
    code: 'GA',
    continent: 'Africa',
    areaInKm2: 267668,
    population: 2025137,
    capital: 'Libreville',
    name: {
      English: 'Gabon',
      Arabic: 'الغابون',
      Chinese: '加蓬',
      French: 'Gabon',
      Hindi: 'गैबॉन',
      Korean: '가봉',
      Japanese: 'ガボン',
      Russian: 'Габон'
    }
  },
  {
    code: 'GM',
    continent: 'n/a',
    areaInKm2: 'n/a',
    population: 'n/a',
    capital: 'Banjul',
    name: {
      English: 'Gambia',
      Arabic: 'غامبيا',
      Chinese: '冈比亚',
      French: 'Gambie',
      Hindi: 'गाम्बिया',
      Korean: '감비아',
      Japanese: 'ガンビア',
      Russian: 'Гамбия'
    }
  },
  {
    code: 'GE',
    continent: 'Asia',
    areaInKm2: 69700,
    population: 3912061,
    capital: 'Tbilisi',
    name: {
      English: 'Georgia',
      Arabic: 'جورجيا',
      Chinese: '格鲁吉亚',
      French: 'Géorgie',
      Hindi: 'जॉर्जिया',
      Korean: '그루지야',
      Japanese: 'ジョージア州',
      Russian: 'Грузия'
    }
  },
  {
    code: 'DE',
    continent: 'Europe',
    areaInKm2: 357114,
    population: 82114224,
    capital: 'Berlin',
    name: {
      English: 'Germany',
      Arabic: 'ألمانيا',
      Chinese: '德国',
      French: 'Allemagne',
      Hindi: 'जर्मनी',
      Korean: '독일',
      Japanese: 'ドイツ',
      Russian: 'Германия'
    }
  },
  {
    code: 'GH',
    continent: 'Africa',
    areaInKm2: 238533,
    population: 28833629,
    capital: 'Accra',
    name: {
      English: 'Ghana',
      Arabic: 'غانا',
      Chinese: '加纳',
      French: 'Ghana',
      Hindi: 'घाना',
      Korean: '가나',
      Japanese: 'ガーナ',
      Russian: 'Гана'
    }
  },
  {
    code: 'GR',
    continent: 'Europe',
    areaInKm2: 131990,
    population: 11159773,
    capital: 'Athens',
    name: {
      English: 'Greece',
      Arabic: 'اليونان',
      Chinese: '希腊',
      French: 'Grèce',
      Hindi: 'यूनान',
      Korean: '그리스',
      Japanese: 'ギリシャ',
      Russian: 'Греция'
    }
  },
  {
    code: 'GL',
    continent: 'Americas',
    areaInKm2: 2166086,
    population: 56480,
    capital: 'Nuuk',
    name: {
      English: 'Greenland',
      Arabic: 'الأرض الخضراء',
      Chinese: '格陵兰',
      French: 'Groenland',
      Hindi: 'ग्रीनलैंड',
      Korean: '그린란드',
      Japanese: 'グリーンランド',
      Russian: 'Гренландия'
    }
  },
  {
    code: 'GD',
    continent: 'Americas',
    areaInKm2: 344,
    population: 107825,
    capital: "St George's",
    name: {
      English: 'Grenada',
      Arabic: 'غرينادا',
      Chinese: '格林纳达',
      French: 'Grenade',
      Hindi: 'ग्रेनेडा',
      Korean: '그레나다',
      Japanese: 'グレナダ',
      Russian: 'Гренада'
    }
  },
  {
    code: 'GT',
    continent: 'Americas',
    areaInKm2: 108889,
    population: 16913503,
    capital: 'Guatemala',
    name: {
      English: 'Guatemala',
      Arabic: 'غواتيمالا',
      Chinese: '危地马拉',
      French: 'Guatemala',
      Hindi: 'ग्वाटेमाला',
      Korean: '과테말라',
      Japanese: 'グアテマラ',
      Russian: 'Гватемала'
    }
  },
  {
    code: 'GN',
    continent: 'Africa',
    areaInKm2: 245857,
    population: 12717176,
    capital: 'Conakry',
    name: {
      English: 'Guinea',
      Arabic: 'غينيا',
      Chinese: '几内亚',
      French: 'Guinée',
      Hindi: 'गिन्नी',
      Korean: '기니',
      Japanese: 'ギニア',
      Russian: 'Гвинея'
    }
  },
  {
    code: 'GW',
    continent: 'Africa',
    areaInKm2: 36125,
    population: 1861283,
    capital: 'Bissau',
    name: {
      English: 'Guinea-Bissau',
      Arabic: 'غينيا بيساو',
      Chinese: '几内亚比绍',
      French: 'Guinée-Bissau',
      Hindi: 'गिनी-बिसाऊ',
      Korean: '기니 비사우',
      Japanese: 'ギニアビサウ',
      Russian: 'Гвинея-Бисау'
    }
  },
  {
    code: 'GY',
    continent: 'Americas',
    areaInKm2: 214969,
    population: 777859,
    capital: 'Georgetown',
    name: {
      English: 'Guyana',
      Arabic: 'غيانا',
      Chinese: '圭亚那',
      French: 'Guyane',
      Hindi: 'गुयाना',
      Korean: '가이아나',
      Japanese: 'ガイアナ',
      Russian: 'Гайана'
    }
  },
  {
    code: 'HT',
    continent: 'Americas',
    areaInKm2: 27750,
    population: 10981229,
    capital: 'Port-au-Prince',
    name: {
      English: 'Haiti',
      Arabic: 'هايتي',
      Chinese: '海地',
      French: 'Haïti',
      Hindi: 'हैती',
      Korean: '아이티',
      Japanese: 'ハイチ',
      Russian: 'Гаити'
    }
  },
  {
    code: 'VA',
    continent: 'Europe',
    areaInKm2: 0.44,
    population: 792,
    capital: '',
    name: {
      English: 'Holy See',
      Arabic: 'الكرسي الرسولي',
      Chinese: '梵蒂冈城',
      French: 'Saint-Siège',
      Hindi: 'पवित्र देखो',
      Korean: '바티칸시국',
      Japanese: 'バチカン市国',
      Russian: 'Святой Престол'
    }
  },
  {
    code: 'HN',
    continent: 'Americas',
    areaInKm2: 112492,
    population: 9265067,
    capital: 'Tegucigalpa',
    name: {
      English: 'Honduras',
      Arabic: 'هندوراس',
      Chinese: '洪都拉斯',
      French: 'Honduras',
      Hindi: 'होंडुरस',
      Korean: '온두라스',
      Japanese: 'ホンジュラス',
      Russian: 'Гондурас'
    }
  },
  {
    code: 'HK',
    continent: 'Asia',
    areaInKm2: 2755,
    population: 7364883,
    capital: 'Hong Kong',
    name: {
      English: 'Hong Kong',
      Arabic: 'هونغ كونغ',
      Chinese: '香港',
      French: 'Hong Kong',
      Hindi: 'हॉगकॉग',
      Korean: '홍콩',
      Japanese: '香港',
      Russian: 'Гонконг'
    }
  },
  {
    code: 'HU',
    continent: 'Europe',
    areaInKm2: 93028,
    population: 9721559,
    capital: 'Budapest',
    name: {
      English: 'Hungary',
      Arabic: 'اليونان',
      Chinese: '匈牙利',
      French: 'Hongrie',
      Hindi: 'हंगरी',
      Korean: '헝가리',
      Japanese: 'ハンガリー',
      Russian: 'Венгрия'
    }
  },
  {
    code: 'IS',
    continent: 'Europe',
    areaInKm2: 103000,
    population: 335025,
    capital: 'Reykjavik',
    name: {
      English: 'Iceland',
      Arabic: 'أيسلندا',
      Chinese: '冰岛',
      French: 'Islande',
      Hindi: 'आइसलैंड',
      Korean: '아이슬란드',
      Japanese: 'アイスランド',
      Russian: 'Исландия'
    }
  },
  {
    code: 'IN',
    continent: 'Asia',
    areaInKm2: 3287263,
    population: 1339180127,
    capital: 'New Delhi',
    name: {
      English: 'India',
      Arabic: 'الهند',
      Chinese: '印度',
      French: 'Inde',
      Hindi: 'इंडिया',
      Korean: '인도',
      Japanese: 'インド',
      Russian: 'Индия'
    }
  },
  {
    code: 'ID',
    continent: 'Asia',
    areaInKm2: 1910931,
    population: 263991379,
    capital: 'Jakarta',
    name: {
      English: 'Indonesia',
      Arabic: 'أندونيسيا',
      Chinese: '印度尼西亚',
      French: 'Indonésie',
      Hindi: 'इंडोनेशिया',
      Korean: '인도네시아 공화국',
      Japanese: 'インドネシア',
      Russian: 'Индонезия'
    }
  },
  {
    code: 'IR',
    continent: 'Asia',
    areaInKm2: 1648195,
    population: 81162788,
    capital: 'Tehran',
    name: {
      English: 'Iran (Islamic Republic of)',
      Arabic: 'إيران (جمهورية - الإسلامية)',
      Chinese: '伊朗伊斯兰共和国',
      French: "Iran (République islamique d ')",
      Hindi: 'ईरान (इस्लामिक रिपब्लिक ऑफ)',
      Korean: '이란 (이슬람 공화국)',
      Japanese: 'イラン（イスラム共和国）',
      Russian: 'Иран (Исламская Республика)'
    }
  },
  {
    code: 'IQ',
    continent: 'Asia',
    areaInKm2: 438317,
    population: 38274618,
    capital: 'Baghdad',
    name: {
      English: 'Iraq',
      Arabic: 'العراق',
      Chinese: '伊拉克',
      French: 'Irak',
      Hindi: 'इराक',
      Korean: '이라크',
      Japanese: 'イラク',
      Russian: 'Ирак'
    }
  },
  {
    code: 'IE',
    continent: 'Europe',
    areaInKm2: 70273,
    population: 4761657,
    capital: 'Dublin',
    name: {
      English: 'Ireland',
      Arabic: 'أيرلندا',
      Chinese: '爱尔兰',
      French: 'Irlande',
      Hindi: 'आयरलैंड',
      Korean: '아일랜드',
      Japanese: 'アイルランド',
      Russian: 'Ирландия'
    }
  },
  {
    code: 'IL',
    continent: 'Asia',
    areaInKm2: 28292,
    population: 8321570,
    capital: 'Jerusalem',
    name: {
      English: 'Israel',
      Arabic: 'إسرائيل',
      Chinese: '以色列',
      French: 'Israël',
      Hindi: 'इजराइल',
      Korean: '이스라엘',
      Japanese: 'イスラエル',
      Russian: 'Израиль'
    }
  },
  {
    code: 'IT',
    continent: 'Europe',
    areaInKm2: 301336,
    population: 59359900,
    capital: 'Rome',
    name: {
      English: 'Italy',
      Arabic: 'إيطاليا',
      Chinese: '意大利',
      French: 'Italie',
      Hindi: 'इटली',
      Korean: '이탈리아',
      Japanese: 'イタリア',
      Russian: 'Италия'
    }
  },
  {
    code: 'JM',
    continent: 'Americas',
    areaInKm2: 10991,
    population: 2890299,
    capital: 'Kingston/Jamaica',
    name: {
      English: 'Jamaica',
      Arabic: 'جامايكا',
      Chinese: '牙买加',
      French: 'Jamaïque',
      Hindi: 'जमैका',
      Korean: '자메이카',
      Japanese: 'ジャマイカ',
      Russian: 'Ямайка'
    }
  },
  {
    code: 'JP',
    continent: 'Asia',
    areaInKm2: 377930,
    population: 127484450,
    capital: 'Tokyo',
    name: {
      English: 'Japan',
      Arabic: 'اليابان',
      Chinese: '日本',
      French: 'Japon',
      Hindi: 'जापान',
      Korean: '일본',
      Japanese: '日本',
      Russian: 'Япония'
    }
  },
  {
    code: 'JO',
    continent: 'Asia',
    areaInKm2: 89342,
    population: 9702353,
    capital: 'Amman',
    name: {
      English: 'Jordan',
      Arabic: 'الأردن',
      Chinese: '约旦',
      French: 'Jordan',
      Hindi: 'जॉर्डन',
      Korean: '요르단',
      Japanese: 'ヨルダン',
      Russian: 'Иордания'
    }
  },
  {
    code: 'KZ',
    continent: 'Asia',
    areaInKm2: 2724900,
    population: 18204499,
    capital: 'Astana',
    name: {
      English: 'Kazakhstan',
      Arabic: 'كازاخستان',
      Chinese: '哈萨克斯坦',
      French: 'Kazakhstan',
      Hindi: 'कजाखस्तान',
      Korean: '카자흐스탄',
      Japanese: 'カザフスタン',
      Russian: 'Казахстан'
    }
  },
  {
    code: 'KE',
    continent: 'Africa',
    areaInKm2: 580367,
    population: 49699862,
    capital: 'Nairobi',
    name: {
      English: 'Kenya',
      Arabic: 'كينيا',
      Chinese: '肯尼亚',
      French: 'Kenya',
      Hindi: 'केन्या',
      Korean: '케냐',
      Japanese: 'ケニア',
      Russian: 'Кения'
    }
  },
  {
    code: 'KI',
    continent: 'Oceania',
    areaInKm2: 811,
    population: 116398,
    capital: 'South Tarawa',
    name: {
      English: 'Kiribati',
      Arabic: 'كيريباس',
      Chinese: '基里巴斯',
      French: 'Kiribati',
      Hindi: 'किरिबाती',
      Korean: '키리바시',
      Japanese: 'キリバス',
      Russian: 'Кирибати'
    }
  },
  {
    code: 'KP',
    continent: 'Asia',
    areaInKm2: 120540,
    population: 25490965,
    capital: 'Pyongyang',
    name: {
      English: "Korea (Democratic People's Republic of)",
      Arabic: 'كوريا، الجمهورية الشعبية الديمقراطية)',
      Chinese: '朝鲜（朝鲜民主主义人民共和国）',
      French: 'République populaire démocratique de Corée)',
      Hindi: 'कोरिया प्रजातात्रिक जनवादी गणतंत्र)',
      Korean: '한국 (조선 민주주의 인민 공화국)',
      Japanese: '韓国（民主党）',
      Russian: 'Корея (Демократическая Народная Республика)'
    }
  },
  {
    code: 'KR',
    continent: 'Asia',
    areaInKm2: 100210,
    population: 50982212,
    capital: 'Seoul',
    name: {
      English: 'Korea (Republic of)',
      Arabic: 'جمهورية كوريا)',
      Chinese: '韩国（共和国）',
      French: 'Corée',
      Hindi: 'कोरिया गणतंत्र)',
      Korean: '대한민국)',
      Japanese: '大韓民国）',
      Russian: 'Корея'
    }
  },
  {
    code: 'KW',
    continent: 'Asia',
    areaInKm2: 17818,
    population: 4136528,
    capital: 'Kuwait',
    name: {
      English: 'Kuwait',
      Arabic: 'الكويت',
      Chinese: '科威特',
      French: 'Koweit',
      Hindi: 'कुवैट',
      Korean: '쿠웨이트',
      Japanese: 'クウェート',
      Russian: 'Кувейт'
    }
  },
  {
    code: 'KG',
    continent: 'Asia',
    areaInKm2: 199951,
    population: 6045117,
    capital: 'Bishkek',
    name: {
      English: 'Kyrgyzstan',
      Arabic: 'قرغيزستان',
      Chinese: '吉尔吉斯斯坦',
      French: 'Kirghizistan',
      Hindi: 'किर्गिज़स्तान',
      Korean: '키르기즈스탄',
      Japanese: 'キルギス',
      Russian: 'Киргизия'
    }
  },
  {
    code: 'LA',
    continent: 'Asia',
    areaInKm2: 236800,
    population: 6858160,
    capital: 'Vientiane',
    name: {
      English: "Lao People's Democratic Republic",
      Arabic: 'جمهورية لاو الديمقراطية الشعبية',
      Chinese: '老挝人民民主共和国',
      French: 'République démocratique populaire lao',
      Hindi: 'लाओ पीपल्स डेमोक्रेटिक रिपब्लिक',
      Korean: '라오스 인민 민주주의 공화국',
      Japanese: 'ラオス人民民主共和国',
      Russian: 'Лаосская Народно-Демократическая Республика'
    }
  },
  {
    code: 'LV',
    continent: 'Europe',
    areaInKm2: 64559,
    population: 1949670,
    capital: 'Riga',
    name: {
      English: 'Latvia',
      Arabic: 'لاتفيا',
      Chinese: '拉脱维亚',
      French: 'Lettonie',
      Hindi: 'लातविया',
      Korean: '라트비아',
      Japanese: 'ラトビア',
      Russian: 'Латвия'
    }
  },
  {
    code: 'LB',
    continent: 'Asia',
    areaInKm2: 10452,
    population: 6082357,
    capital: 'Beirut',
    name: {
      English: 'Lebanon',
      Arabic: 'لبنان',
      Chinese: '黎巴嫩',
      French: 'Liban',
      Hindi: 'लेबनान',
      Korean: '레바논',
      Japanese: 'レバノン',
      Russian: 'Ливан'
    }
  },
  {
    code: 'LS',
    continent: 'Africa',
    areaInKm2: 30355,
    population: 2233339,
    capital: 'Maseru',
    name: {
      English: 'Lesotho',
      Arabic: 'ليسوتو',
      Chinese: '莱索托',
      French: 'Lesotho',
      Hindi: 'लिसोटो',
      Korean: '레소토',
      Japanese: 'レソト',
      Russian: 'Лесото'
    }
  },
  {
    code: 'LR',
    continent: 'Africa',
    areaInKm2: 111369,
    population: 4731906,
    capital: 'Monrovia',
    name: {
      English: 'Liberia',
      Arabic: 'ليبيريا',
      Chinese: '利比里亚',
      French: 'Libéria',
      Hindi: 'लाइबेरिया',
      Korean: '라이베리아',
      Japanese: 'リベリア',
      Russian: 'Либерия'
    }
  },
  {
    code: 'LY',
    continent: 'Africa',
    areaInKm2: 1759540,
    population: 6374616,
    capital: 'Tripoli',
    name: {
      English: 'Libya',
      Arabic: 'ليبيا',
      Chinese: '利比亚',
      French: 'Libye',
      Hindi: 'लीबिया',
      Korean: '리비아',
      Japanese: 'リビア',
      Russian: 'Ливия'
    }
  },
  {
    code: 'LI',
    continent: 'Europe',
    areaInKm2: 160,
    population: 37922,
    capital: 'Vaduz',
    name: {
      English: 'Liechtenstein',
      Arabic: 'ليختنشتاين',
      Chinese: '列支敦士登',
      French: 'Liechtenstein',
      Hindi: 'लिकटेंस्टीन',
      Korean: '리히텐슈타인',
      Japanese: 'リヒテンシュタイン',
      Russian: 'Лихтенштейн'
    }
  },
  {
    code: 'LT',
    continent: 'Europe',
    areaInKm2: 65300,
    population: 2890297,
    capital: 'Vilnius',
    name: {
      English: 'Lithuania',
      Arabic: 'ليتوانيا',
      Chinese: '立陶宛',
      French: 'Lituanie',
      Hindi: 'लिथुआनिया',
      Korean: '리투아니아',
      Japanese: 'リトアニア',
      Russian: 'Литва'
    }
  },
  {
    code: 'LU',
    continent: 'Europe',
    areaInKm2: 2586,
    population: 583455,
    capital: 'Luxembourg City',
    name: {
      English: 'Luxembourg',
      Arabic: 'لوكسمبورغ',
      Chinese: '卢森堡',
      French: 'Luxembourg',
      Hindi: 'लक्समबर्ग',
      Korean: '룩셈부르크',
      Japanese: 'ルクセンブルク',
      Russian: 'Люксембург'
    }
  },
  {
    code: 'MO',
    continent: 'Asia',
    areaInKm2: 31.3,
    population: 622567,
    capital: 'Macao',
    name: {
      English: 'Macao',
      Arabic: 'ماكاو',
      Chinese: '澳门',
      French: 'Macao',
      Hindi: 'मकाओ',
      Korean: '마카오',
      Japanese: 'マカオ',
      Russian: 'Macao'
    }
  },
  {
    code: 'MK',
    continent: 'n/a',
    areaInKm2: 25713,
    population: 'n/a',
    capital: 'Skopje',
    name: {
      English: 'Macedonia ',
      Arabic: 'مقدونيا ',
      Chinese: '马其顿',
      French: 'Macédoine',
      Hindi: 'मैसेडोनिया',
      Korean: '마케도니아',
      Japanese: 'マケドニア',
      Russian: 'Македония'
    }
  },
  {
    code: 'MG',
    continent: 'Africa',
    areaInKm2: 587041,
    population: 25570895,
    capital: 'Antananarivo',
    name: {
      English: 'Madagascar',
      Arabic: 'مدغشقر',
      Chinese: '马达加斯加',
      French: 'Madagascar',
      Hindi: 'मेडागास्कर',
      Korean: '마다가스카르',
      Japanese: 'マダガスカル',
      Russian: 'Мадагаскар'
    }
  },
  {
    code: 'MW',
    continent: 'Africa',
    areaInKm2: 118484,
    population: 18622104,
    capital: 'Lilongwe',
    name: {
      English: 'Malawi',
      Arabic: 'مالاوي',
      Chinese: '马拉维',
      French: 'Malawi',
      Hindi: 'मलावी',
      Korean: '말라위',
      Japanese: 'マラウイ',
      Russian: 'Малави'
    }
  },
  {
    code: 'MY',
    continent: 'Asia',
    areaInKm2: 330803,
    population: 31624264,
    capital: 'Kuala Lumpur',
    name: {
      English: 'Malaysia',
      Arabic: 'ماليزيا',
      Chinese: '马来西亚',
      French: 'Malaisie',
      Hindi: 'मलेशिया',
      Korean: '말레이시아',
      Japanese: 'マレーシア',
      Russian: 'Малайзия'
    }
  },
  {
    code: 'MV',
    continent: 'Asia',
    areaInKm2: 300,
    population: 436330,
    capital: 'Malé',
    name: {
      English: 'Maldives',
      Arabic: 'جزر المالديف',
      Chinese: '马尔代夫',
      French: 'Maldives',
      Hindi: 'मालदीव',
      Korean: '몰디브',
      Japanese: 'モルディブ',
      Russian: 'Мальдивы'
    }
  },
  {
    code: 'ML',
    continent: 'Africa',
    areaInKm2: 1240192,
    population: 18541980,
    capital: 'Bamako',
    name: {
      English: 'Mali',
      Arabic: 'مالي',
      Chinese: '马里',
      French: 'Mali',
      Hindi: 'माली',
      Korean: '말리',
      Japanese: 'マリ',
      Russian: 'Мали'
    }
  },
  {
    code: 'MT',
    continent: 'Europe',
    areaInKm2: 316,
    population: 430835,
    capital: 'Valletta',
    name: {
      English: 'Malta',
      Arabic: 'مالطا',
      Chinese: '马耳他',
      French: 'Malte',
      Hindi: 'माल्टा',
      Korean: '몰타',
      Japanese: 'マルタ',
      Russian: 'Мальта'
    }
  },
  {
    code: 'MH',
    continent: 'Oceania',
    areaInKm2: 181,
    population: 53127,
    capital: 'Majuro',
    name: {
      English: 'Marshall Islands',
      Arabic: 'جزر مارشال',
      Chinese: '马绍尔群岛',
      French: 'Iles Marshall',
      Hindi: 'मार्शल द्वीप समूह',
      Korean: '마샬 군도',
      Japanese: 'マーシャル諸島',
      Russian: 'Маршалловы острова'
    }
  },
  {
    code: 'MR',
    continent: 'Africa',
    areaInKm2: 1030700,
    population: 4420184,
    capital: 'Nouakchott',
    name: {
      English: 'Mauritania',
      Arabic: 'موريتانيا',
      Chinese: '毛里塔尼亚',
      French: 'Mauritanie',
      Hindi: 'मॉरिटानिया',
      Korean: '모리타니',
      Japanese: 'モーリタニア',
      Russian: 'Мавритания'
    }
  },
  {
    code: 'MU',
    continent: 'Africa',
    areaInKm2: 2040,
    population: 1265138,
    capital: 'Port Louis',
    name: {
      English: 'Mauritius',
      Arabic: 'موريشيوس',
      Chinese: '毛里求斯',
      French: 'Maurice',
      Hindi: 'मॉरीशस',
      Korean: '모리셔스',
      Japanese: 'モーリシャス',
      Russian: 'Маврикий'
    }
  },
  {
    code: 'MX',
    continent: 'Americas',
    areaInKm2: 1964375,
    population: 129163276,
    capital: 'Mexico City',
    name: {
      English: 'Mexico',
      Arabic: 'المكسيك',
      Chinese: '墨西哥',
      French: 'Mexique',
      Hindi: 'मेक्सिको',
      Korean: '멕시코',
      Japanese: 'メキシコ',
      Russian: 'Мексика'
    }
  },
  {
    code: 'FM',
    continent: '',
    areaInKm2: 702,
    population: '',
    capital: '',
    name: {
      English: 'Micronesia (Federated States of)',
      Arabic: 'ميكرونيزيا (ولايات - الموحدة)',
      Chinese: '密克罗尼西亚联邦',
      French: 'Micronésie (États fédérés de)',
      Hindi: 'माइक्रोनेशिया (संघीय राज्य)',
      Korean: '미크로네시아 (연방)',
      Japanese: 'ミクロネシア（連邦）',
      Russian: 'Микронезия (Федеративные Штаты)'
    }
  },
  {
    code: 'MD',
    continent: 'Europe',
    areaInKm2: 33846,
    population: 4051212,
    capital: 'Chisinau',
    name: {
      English: 'Moldova (Republic of)',
      Arabic: 'جمهورية مولدوفا',
      Chinese: '摩尔多瓦共和国',
      French: 'Moldavie (République de)',
      Hindi: 'मोल्दोवा (गणराज्य)',
      Korean: '몰도바 (공화국)',
      Japanese: 'モルドバ共和国',
      Russian: 'Молдова (Республика)'
    }
  },
  {
    code: 'MC',
    continent: 'Europe',
    areaInKm2: 2.02,
    population: 38695,
    capital: 'Monaco',
    name: {
      English: 'Monaco',
      Arabic: 'موناكو',
      Chinese: '摩纳哥',
      French: 'Monaco',
      Hindi: 'मोनाको',
      Korean: '모나코',
      Japanese: 'モナコ',
      Russian: 'Монако'
    }
  },
  {
    code: 'MN',
    continent: 'Asia',
    areaInKm2: 1564110,
    population: 3075647,
    capital: 'Ulaanbaatar',
    name: {
      English: 'Mongolia',
      Arabic: 'منغوليا',
      Chinese: '蒙古',
      French: 'Mongolie',
      Hindi: 'मंगोलिया',
      Korean: '몽골리아',
      Japanese: 'モンゴル',
      Russian: 'Монголия'
    }
  },
  {
    code: 'ME',
    continent: 'Europe',
    areaInKm2: 13812,
    population: 628960,
    capital: 'Podgorica',
    name: {
      English: 'Montenegro',
      Arabic: 'الجبل الأسود',
      Chinese: '黑山',
      French: 'Monténégro',
      Hindi: 'मोंटेनेग्रो',
      Korean: '몬테네그로',
      Japanese: 'モンテネグロ',
      Russian: 'Черногория'
    }
  },
  {
    code: 'MA',
    continent: 'Africa',
    areaInKm2: 446550,
    population: 35739580,
    capital: 'Rabat',
    name: {
      English: 'Morocco',
      Arabic: 'المغرب',
      Chinese: '摩洛哥',
      French: 'Maroc',
      Hindi: 'मोरक्को',
      Korean: '모로코',
      Japanese: 'モロッコ',
      Russian: 'Марокко'
    }
  },
  {
    code: 'MZ',
    continent: 'Africa',
    areaInKm2: 801590,
    population: 29668834,
    capital: 'Maputo',
    name: {
      English: 'Mozambique',
      Arabic: 'موزمبيق',
      Chinese: '莫桑比克',
      French: 'Mozambique',
      Hindi: 'मोजाम्बिक',
      Korean: '모잠비크',
      Japanese: 'モザンビーク',
      Russian: 'Мозамбик'
    }
  },
  {
    code: 'MM',
    continent: 'Asia',
    areaInKm2: 676578,
    population: 53370609,
    capital: 'Naypyidaw',
    name: {
      English: 'Myanmar',
      Arabic: 'ميانمار',
      Chinese: '缅甸',
      French: 'Myanmar',
      Hindi: 'म्यांमार',
      Korean: '미얀마',
      Japanese: 'ミャンマー',
      Russian: 'Мьянма'
    }
  },
  {
    code: 'NA',
    continent: 'Africa',
    areaInKm2: 825615,
    population: 2533794,
    capital: 'Windhoek',
    name: {
      English: 'Namibia',
      Arabic: 'ناميبيا',
      Chinese: '纳米比亚',
      French: 'Namibie',
      Hindi: 'नामीबिया',
      Korean: '나미비아',
      Japanese: 'ナミビア',
      Russian: 'Намибия'
    }
  },
  {
    code: 'NR',
    continent: 'Oceania',
    areaInKm2: 21,
    population: 11359,
    capital: 'Yaren',
    name: {
      English: 'Nauru',
      Arabic: 'ناورو',
      Chinese: '瑙鲁',
      French: 'Nauru',
      Hindi: 'नाउरू',
      Korean: '나우루',
      Japanese: 'ナウル',
      Russian: 'Науру'
    }
  },
  {
    code: 'NP',
    continent: 'Asia',
    areaInKm2: 147181,
    population: 29304998,
    capital: 'Kathmandu',
    name: {
      English: 'Nepal',
      Arabic: 'نيبال',
      Chinese: '尼泊尔',
      French: 'Népal',
      Hindi: 'नेपाल',
      Korean: '네팔',
      Japanese: 'ネパール',
      Russian: 'Непал'
    }
  },
  {
    code: 'NL',
    continent: 'Europe',
    areaInKm2: 41850,
    population: 17035938,
    capital: 'Amsterdam/The Hague',
    name: {
      English: 'Netherlands',
      Arabic: 'هولندا',
      Chinese: '荷兰',
      French: 'Pays-Bas',
      Hindi: 'नीदरलैंड्स',
      Korean: '네덜란드',
      Japanese: 'オランダ',
      Russian: 'Нидерланды'
    }
  },
  {
    code: 'NZ',
    continent: 'Oceania',
    areaInKm2: 270467,
    population: 4705818,
    capital: 'Wellington',
    name: {
      English: 'New Zealand',
      Arabic: 'نيوزيلاندا',
      Chinese: '新西兰',
      French: 'Nouvelle-Zélande',
      Hindi: 'न्यूजीलैंड',
      Korean: '뉴질랜드',
      Japanese: 'ニュージーランド',
      Russian: 'Новая Зеландия'
    }
  },
  {
    code: 'NI',
    continent: 'Americas',
    areaInKm2: 130373,
    population: 6217581,
    capital: 'Managua',
    name: {
      English: 'Nicaragua',
      Arabic: 'نيكاراغوا',
      Chinese: '尼加拉瓜',
      French: 'Nicaragua',
      Hindi: 'निकारागुआ',
      Korean: '니카라과',
      Japanese: 'ニカラグア',
      Russian: 'Никарагуа'
    }
  },
  {
    code: 'NE',
    continent: 'Africa',
    areaInKm2: 1267000,
    population: 21477348,
    capital: 'Niamey',
    name: {
      English: 'Niger',
      Arabic: 'النيجر',
      Chinese: '尼日尔',
      French: 'Niger',
      Hindi: 'नाइजर',
      Korean: '니제르',
      Japanese: 'ニジェール',
      Russian: 'Нигер'
    }
  },
  {
    code: 'NG',
    continent: 'Africa',
    areaInKm2: 923768,
    population: 190886311,
    capital: 'Abuja',
    name: {
      English: 'Nigeria',
      Arabic: 'نيجيريا',
      Chinese: '尼日利亚',
      French: 'Nigeria',
      Hindi: 'नाइजीरिया',
      Korean: '나이지리아',
      Japanese: 'ナイジェリア',
      Russian: 'Нигерия'
    }
  },
  {
    code: 'NO',
    continent: 'Europe',
    areaInKm2: 323802,
    population: 5305383,
    capital: 'Oslo',
    name: {
      English: 'Norway',
      Arabic: 'النرويج',
      Chinese: '挪威',
      French: 'Norvège',
      Hindi: 'नॉर्वे',
      Korean: '노르웨이',
      Japanese: 'ノルウェー',
      Russian: 'Норвегия'
    }
  },
  {
    code: 'OM',
    continent: 'Asia',
    areaInKm2: 309500,
    population: 4636262,
    capital: 'Muscat',
    name: {
      English: 'Oman',
      Arabic: 'سلطنة عمان',
      Chinese: '阿曼',
      French: 'Oman',
      Hindi: 'ओमान',
      Korean: '오만',
      Japanese: 'オマーン',
      Russian: 'Оман'
    }
  },
  {
    code: 'PK',
    continent: 'Asia',
    areaInKm2: 881912,
    population: 197015955,
    capital: 'Islamabad',
    name: {
      English: 'Pakistan',
      Arabic: 'باكستان',
      Chinese: '巴基斯坦',
      French: 'Pakistan',
      Hindi: 'पाकिस्तान',
      Korean: '파키스탄',
      Japanese: 'パキスタン',
      Russian: 'Пакистан'
    }
  },
  {
    code: 'PW',
    continent: 'Oceania',
    areaInKm2: 459,
    population: 21729,
    capital: 'Koror',
    name: {
      English: 'Palau',
      Arabic: 'بالاو',
      Chinese: '帕劳',
      French: 'Palau',
      Hindi: 'पलाऊ',
      Korean: '팔라우',
      Japanese: 'パラオ',
      Russian: 'Palau'
    }
  },
  {
    code: 'PS',
    continent: 'Asia',
    areaInKm2: 6020,
    population: 4920724,
    capital: 'Gaza',
    name: {
      English: 'Palestine',
      Arabic: 'فلسطين، ستات أوف',
      Chinese: '巴勒斯坦，',
      French: 'Palestine',
      Hindi: 'फिलिस्तीन',
      Korean: '팔레스타인',
      Japanese: 'パレスチナ',
      Russian: 'Палестина'
    }
  },
  {
    code: 'PA',
    continent: 'Americas',
    areaInKm2: 75417,
    population: 4098587,
    capital: 'Panama City',
    name: {
      English: 'Panama',
      Arabic: 'بناما',
      Chinese: '巴拿马',
      French: 'Panama',
      Hindi: 'पनामा',
      Korean: '파나마',
      Japanese: 'パナマ',
      Russian: 'Панама'
    }
  },
  {
    code: 'PG',
    continent: 'Oceania',
    areaInKm2: 462840,
    population: 8251162,
    capital: 'Port Moresby',
    name: {
      English: 'Papua New Guinea',
      Arabic: 'بابوا غينيا الجديدة',
      Chinese: '巴布亚新几内亚',
      French: 'Papouasie Nouvelle Guinée',
      Hindi: 'पापुआ न्यू गिनी',
      Korean: '파푸아 뉴기니',
      Japanese: 'パプアニューギニア',
      Russian: 'Папуа - Новая Гвинея'
    }
  },
  {
    code: 'PY',
    continent: 'Americas',
    areaInKm2: 406752,
    population: 6811297,
    capital: 'Asuncion',
    name: {
      English: 'Paraguay',
      Arabic: 'باراغواي',
      Chinese: '巴拉圭',
      French: 'Paraguay',
      Hindi: 'परागुआ',
      Korean: '파라과이',
      Japanese: 'パラグアイ',
      Russian: 'Парагвай'
    }
  },
  {
    code: 'PE',
    continent: 'Americas',
    areaInKm2: 1285216,
    population: 32165485,
    capital: 'Lima',
    name: {
      English: 'Peru',
      Arabic: 'بيرو',
      Chinese: '秘鲁',
      French: 'Pérou',
      Hindi: 'पेरू',
      Korean: '페루',
      Japanese: 'ペルー',
      Russian: 'Перу'
    }
  },
  {
    code: 'PH',
    continent: 'Asia',
    areaInKm2: 300000,
    population: 104918090,
    capital: 'Manila',
    name: {
      English: 'Philippines',
      Arabic: 'الفلبين',
      Chinese: '菲律宾',
      French: 'Philippines',
      Hindi: 'फिलीपींस',
      Korean: '필리핀 제도',
      Japanese: 'フィリピン',
      Russian: 'Филиппины'
    }
  },
  {
    code: 'PL',
    continent: 'Europe',
    areaInKm2: 312679,
    population: 38170712,
    capital: 'Warsaw',
    name: {
      English: 'Poland',
      Arabic: 'بولندا',
      Chinese: '波兰',
      French: 'Pologne',
      Hindi: 'पोलैंड',
      Korean: '폴란드',
      Japanese: 'ポーランド',
      Russian: 'Польша'
    }
  },
  {
    code: 'PT',
    continent: 'Europe',
    areaInKm2: 92090,
    population: 10329506,
    capital: 'Lisbon',
    name: {
      English: 'Portugal',
      Arabic: 'البرتغال',
      Chinese: '葡萄牙',
      French: 'Portugal',
      Hindi: 'पुर्तगाल',
      Korean: '포르투갈',
      Japanese: 'ポルトガル',
      Russian: 'Португалия'
    }
  },
  {
    code: 'PR',
    continent: 'Americas',
    areaInKm2: 9104,
    population: 3663131,
    capital: 'San Juan',
    name: {
      English: 'Puerto Rico',
      Arabic: 'بورتوريكو',
      Chinese: '波多黎各',
      French: 'Porto Rico',
      Hindi: 'प्यूर्टो रिको',
      Korean: '푸에르토 리코',
      Japanese: 'プエルトリコ',
      Russian: 'Пуэрто-Рико'
    }
  },
  {
    code: 'QA',
    continent: 'Asia',
    areaInKm2: 11586,
    population: 2639211,
    capital: 'Doha',
    name: {
      English: 'Qatar',
      Arabic: 'دولة قطر',
      Chinese: '卡塔尔',
      French: 'Qatar',
      Hindi: 'कतर',
      Korean: '카타르',
      Japanese: 'カタール',
      Russian: 'Катар'
    }
  },
  {
    code: 'RO',
    continent: 'Europe',
    areaInKm2: 238397,
    population: 19679306,
    capital: 'Bucharest',
    name: {
      English: 'Romania',
      Arabic: 'رومانيا',
      Chinese: '罗马尼亚',
      French: 'Roumanie',
      Hindi: 'रोमानिया',
      Korean: '루마니아',
      Japanese: 'ルーマニア',
      Russian: 'Румыния'
    }
  },
  {
    code: 'RU',
    continent: 'Europe',
    areaInKm2: 17098246,
    population: 143989754,
    capital: 'Moscow',
    name: {
      English: 'Russian Federation',
      Arabic: 'الاتحاد الروسي',
      Chinese: '俄罗斯联邦',
      French: 'Fédération Russe',
      Hindi: 'रूसी संघ',
      Korean: '러시아 연방',
      Japanese: 'ロシア連邦',
      Russian: 'Российская Федерация'
    }
  },
  {
    code: 'RW',
    continent: 'Africa',
    areaInKm2: 26338,
    population: 12208407,
    capital: 'Kigali',
    name: {
      English: 'Rwanda',
      Arabic: 'رواندا',
      Chinese: '卢旺达',
      French: 'Rwanda',
      Hindi: 'रवांडा',
      Korean: '르완다',
      Japanese: 'ルワンダ',
      Russian: 'Руанда'
    }
  },
  {
    code: 'SH',
    continent: 'Africa',
    areaInKm2: 308,
    population: 4049,
    capital: '',
    name: {
      English: 'Saint Helena and Ascension and Tristan da Cunha',
      Arabic: 'سانت هيلينا، أسينسيون وتريستان دا كونها',
      Chinese: '圣赫勒拿，阿森松和特里斯坦达库尼亚',
      French: 'Sainte-Hélène Ascension et Tristan da Cunha',
      Hindi: 'सेंट हेलेना असेंशन और त्रिस्तान दा कुन्हा',
      Korean: '세인트 헬레나 승천 및 트리스탄 다 쿤하',
      Japanese: 'セントヘレナ、アセンション、トリスタンダクーニャ',
      Russian: 'Святой Елены Вознесения и Тристана-да-Куньи'
    }
  },
  {
    code: 'KN',
    continent: 'Americas',
    areaInKm2: 261,
    population: 55345,
    capital: 'Basseterre',
    name: {
      English: 'Saint Kitts and Nevis',
      Arabic: 'سانت كيتس ونيفيس',
      Chinese: '圣基茨和尼维斯',
      French: 'Saint-Christophe-et-Niévès',
      Hindi: 'संत किट्ट्स और नेविस',
      Korean: '세인트 키츠 네비스',
      Japanese: 'セントクリストファー・ネイビス',
      Russian: 'Сент-Китс и Невис'
    }
  },
  {
    code: 'LC',
    continent: 'Americas',
    areaInKm2: 616,
    population: 178844,
    capital: 'Castries',
    name: {
      English: 'Saint Lucia',
      Arabic: 'القديسة لوسيا',
      Chinese: '圣卢西亚',
      French: 'Sainte-Lucie',
      Hindi: 'सेंट लूसिया',
      Korean: '세인트 루시아',
      Japanese: 'セントルシア',
      Russian: 'Санкт-Люсия'
    }
  },
  {
    code: 'MF',
    continent: '',
    areaInKm2: 54,
    population: '',
    capital: 'Philipsburg',
    name: {
      English: 'Saint Martin (French part)',
      Arabic: 'سانت مارتن (الجزء الفرنسي)',
      Chinese: '圣马丁（法语部分）',
      French: 'Saint Martin (partie française)',
      Hindi: 'सेंट मार्टिन (फ्रेंच भाग)',
      Korean: '세인트 마틴 (프랑스어 부분)',
      Japanese: 'サンマルタン（フランス語部分）',
      Russian: 'Сен-Мартен (французская часть)'
    }
  },
  {
    code: 'VC',
    continent: 'Americas',
    areaInKm2: 242,
    population: 109897,
    capital: 'Kingstown',
    name: {
      English: 'Saint Vincent and the Grenadines',
      Arabic: 'سانت فنسنت وجزر غرينادين',
      Chinese: '圣文森特和格林纳丁斯',
      French: 'Saint-Vincent-et-les-Grenadines',
      Hindi: 'संत विंसेंट अँड थे ग्रेनडीनेस',
      Korean: '세인트 빈센트 그레나딘',
      Japanese: 'セントビンセントとグレナディーン',
      Russian: 'Святой Винсент и Гренадины'
    }
  },
  {
    code: 'WS',
    continent: 'Oceania',
    areaInKm2: 2842,
    population: 196440,
    capital: 'Apia',
    name: {
      English: 'Samoa',
      Arabic: 'ساموا',
      Chinese: '萨摩亚',
      French: 'Samoa',
      Hindi: 'समोआ',
      Korean: '사모아',
      Japanese: 'サモア',
      Russian: 'Самоа'
    }
  },
  {
    code: 'SM',
    continent: 'Europe',
    areaInKm2: 61,
    population: 33400,
    capital: 'San Marino',
    name: {
      English: 'San Marino',
      Arabic: 'سان مارينو',
      Chinese: '圣马力诺',
      French: 'Saint Marin',
      Hindi: 'सैन मैरीनो',
      Korean: '산 마리노',
      Japanese: 'サンマリノ',
      Russian: 'Сан-Марино'
    }
  },
  {
    code: 'ST',
    continent: 'Africa',
    areaInKm2: 964,
    population: 204327,
    capital: 'São Tomé',
    name: {
      English: 'Sao Tome and Principe',
      Arabic: 'ساو تومي وبرينسيبي',
      Chinese: '圣多美和普林西比',
      French: 'Sao Tomé-et-Principe',
      Hindi: 'साओ टोमे और प्रिंसिपे',
      Korean: '상투 메 프린시 페',
      Japanese: 'サントメプリンシペ',
      Russian: 'Сан-Томе и Принсипи'
    }
  },
  {
    code: 'SA',
    continent: 'Asia',
    areaInKm2: 2149690,
    population: 32938213,
    capital: 'Riyadh',
    name: {
      English: 'Saudi Arabia',
      Arabic: 'المملكة العربية السعودية',
      Chinese: '沙特阿拉伯',
      French: 'Arabie Saoudite',
      Hindi: 'सऊदी अरब',
      Korean: '사우디 아라비아',
      Japanese: 'サウジアラビア',
      Russian: 'Саудовская Аравия'
    }
  },
  {
    code: 'SN',
    continent: 'Africa',
    areaInKm2: 196722,
    population: 15850567,
    capital: 'Dakar',
    name: {
      English: 'Senegal',
      Arabic: 'السنغال',
      Chinese: '塞内加尔',
      French: 'Sénégal',
      Hindi: 'सेनेगल',
      Korean: '세네갈',
      Japanese: 'セネガル',
      Russian: 'Сенегал'
    }
  },
  {
    code: 'RS',
    continent: 'Europe',
    areaInKm2: 88361,
    population: 8790574,
    capital: 'Belgrade',
    name: {
      English: 'Serbia',
      Arabic: 'صربيا',
      Chinese: '塞尔维亚',
      French: 'Serbie',
      Hindi: 'सर्बिया',
      Korean: '세르비아',
      Japanese: 'セルビア',
      Russian: 'Сербия'
    }
  },
  {
    code: 'SC',
    continent: 'Africa',
    areaInKm2: 452,
    population: 94737,
    capital: 'Victoria/Seychelles',
    name: {
      English: 'Seychelles',
      Arabic: 'سيشيل',
      Chinese: '塞舌尔',
      French: 'les Seychelles',
      Hindi: 'सेशेल्स',
      Korean: '세이셸',
      Japanese: 'セイシェル',
      Russian: 'Сейшельские острова'
    }
  },
  {
    code: 'SL',
    continent: 'Africa',
    areaInKm2: 71740,
    population: 7557212,
    capital: 'Freetown',
    name: {
      English: 'Sierra Leone',
      Arabic: 'سيرا ليون',
      Chinese: '塞拉利昂',
      French: 'Sierra Leone',
      Hindi: 'सियरा लिओन',
      Korean: '시에라 리온',
      Japanese: 'シエラレオネ',
      Russian: 'Сьерра-Леоне'
    }
  },
  {
    code: 'SG',
    continent: 'Asia',
    areaInKm2: 719,
    population: 5708844,
    capital: 'Singapore',
    name: {
      English: 'Singapore',
      Arabic: 'سنغافورة',
      Chinese: '新加坡',
      French: 'Singapour',
      Hindi: 'सिंगापुर',
      Korean: '싱가포르',
      Japanese: 'シンガポール',
      Russian: 'Сингапур'
    }
  },
  {
    code: 'SX',
    continent: 'Americas',
    areaInKm2: 34,
    population: 40120,
    capital: 'Philipsburg',
    name: {
      English: 'Sint Maarten (Dutch part)',
      Arabic: 'سانت مارتن (الجزء الهولندي)',
      Chinese: '圣马丁（荷兰部分）',
      French: 'Sint Maarten (partie néerlandaise)',
      Hindi: 'सिंट मार्टेन (डच भाग)',
      Korean: '신트 마틴 (네덜란드어 부분)',
      Japanese: 'シント・マールテン（オランダ語）',
      Russian: 'Синт-Маартен (голландская часть)'
    }
  },
  {
    code: 'SK',
    continent: 'Europe',
    areaInKm2: 49037,
    population: 5447662,
    capital: 'Bratislava',
    name: {
      English: 'Slovakia',
      Arabic: 'سلوفاكيا',
      Chinese: '斯洛伐克',
      French: 'Slovaquie',
      Hindi: 'स्लोवाकिया',
      Korean: '슬로바키아',
      Japanese: 'スロバキア',
      Russian: 'Словакия'
    }
  },
  {
    code: 'SI',
    continent: 'Europe',
    areaInKm2: 20273,
    population: 2079976,
    capital: 'Ljubljana',
    name: {
      English: 'Slovenia',
      Arabic: 'سلوفينيا',
      Chinese: '斯洛文尼亚',
      French: 'La Slovénie',
      Hindi: 'स्लोवेनिया',
      Korean: '슬로베니아',
      Japanese: 'スロベニア',
      Russian: 'Словения'
    }
  },
  {
    code: 'SB',
    continent: 'Oceania',
    areaInKm2: 28896,
    population: 611343,
    capital: 'Honiara',
    name: {
      English: 'Solomon Islands',
      Arabic: 'جزر سليمان',
      Chinese: '所罗门群岛',
      French: 'Les îles Salomon',
      Hindi: 'सोलोमन इस्लैंडस',
      Korean: '솔로몬 제도',
      Japanese: 'ソロモン諸島',
      Russian: 'Соломоновы острова'
    }
  },
  {
    code: 'SO',
    continent: 'Africa',
    areaInKm2: 637657,
    population: 14742523,
    capital: 'Mogadishu',
    name: {
      English: 'Somalia',
      Arabic: 'الصومال',
      Chinese: '索马里',
      French: 'Somalie',
      Hindi: 'सोमालिया',
      Korean: '소말리아',
      Japanese: 'ソマリア',
      Russian: 'Сомали'
    }
  },
  {
    code: 'ZA',
    continent: 'Africa',
    areaInKm2: 1221037,
    population: 56717156,
    capital: 'Pretoria',
    name: {
      English: 'South Africa',
      Arabic: 'جنوب أفريقيا',
      Chinese: '南非',
      French: 'Afrique du Sud',
      Hindi: 'दक्षिण अफ्रीका',
      Korean: '남아프리카',
      Japanese: '南アフリカ',
      Russian: 'Южная Африка'
    }
  },
  {
    code: 'GS',
    continent: '',
    areaInKm2: 3903,
    population: '',
    capital: '',
    name: {
      English: 'South Georgia and the South Sandwich Islands',
      Arabic: 'جورجيا الجنوبية وجزر ساندويتش الجنوبية',
      Chinese: '南乔治亚和南桑威奇群岛',
      French: 'Géorgie du Sud et les îles Sandwich du Sud',
      Hindi: 'दक्षिण जॉर्जिया और दक्षिण सैंडविच द्वीप समूह',
      Korean: '사우스 조지아 및 사우스 샌드위치 제도',
      Japanese: 'サウスジョージア州とサウス・サンドイッチ諸島',
      Russian: 'Южная Георгия и Южные Сандвичевы острова'
    }
  },
  {
    code: 'SS',
    continent: 'Africa',
    areaInKm2: 644329,
    population: 12575714,
    capital: 'Juba',
    name: {
      English: 'South Sudan',
      Arabic: 'جنوب السودان',
      Chinese: '南苏丹',
      French: 'Soudan du sud',
      Hindi: 'दक्षिण सूडान',
      Korean: '남 수단',
      Japanese: '南スーダン',
      Russian: 'южный Судан'
    }
  },
  {
    code: 'ES',
    continent: 'Europe',
    areaInKm2: 505992,
    population: 46354321,
    capital: 'Madrid',
    name: {
      English: 'Spain',
      Arabic: 'إسبانيا',
      Chinese: '西班牙',
      French: 'Espagne',
      Hindi: 'स्पेन',
      Korean: '스페인',
      Japanese: 'スペイン',
      Russian: 'Испания'
    }
  },
  {
    code: 'LK',
    continent: 'Asia',
    areaInKm2: 65610,
    population: 20876917,
    capital: 'Sri Jayawardenapura Kotte',
    name: {
      English: 'Sri Lanka',
      Arabic: 'سيريلانكا',
      Chinese: '斯里兰卡',
      French: 'Sri Lanka',
      Hindi: 'श्री लंका',
      Korean: '스리랑카',
      Japanese: 'スリランカ',
      Russian: 'Шри-Ланка'
    }
  },
  {
    code: 'SD',
    continent: 'Africa',
    areaInKm2: 1861484,
    population: 40533330,
    capital: 'Khartoum',
    name: {
      English: 'Sudan',
      Arabic: 'سودان',
      Chinese: '苏丹',
      French: 'Soudan',
      Hindi: 'सूडान',
      Korean: '수단',
      Japanese: 'スーダン',
      Russian: 'Судан'
    }
  },
  {
    code: 'SR',
    continent: 'Americas',
    areaInKm2: 163820,
    population: 563402,
    capital: 'Paramaribo',
    name: {
      English: 'Suriname',
      Arabic: 'سورينام',
      Chinese: '苏里南',
      French: 'Suriname',
      Hindi: 'सूरीनाम',
      Korean: '수리남',
      Japanese: 'スリナム',
      Russian: 'Суринам'
    }
  },
  {
    code: 'SZ',
    continent: 'Africa',
    areaInKm2: 17364,
    population: 1367254,
    capital: 'Mbabane',
    name: {
      English: 'Swaziland',
      Arabic: 'سوازيلاند',
      Chinese: '斯威士兰',
      French: 'Swaziland',
      Hindi: 'स्वाजीलैंड',
      Korean: '스와질란드',
      Japanese: 'スワジランド',
      Russian: 'Свазиленд'
    }
  },
  {
    code: 'SE',
    continent: 'Europe',
    areaInKm2: 450295,
    population: 9910701,
    capital: 'Stockholm',
    name: {
      English: 'Sweden',
      Arabic: 'السويد',
      Chinese: '瑞典',
      French: 'Suède',
      Hindi: 'स्वीडन',
      Korean: '스웨덴',
      Japanese: 'スウェーデン',
      Russian: 'Швеция'
    }
  },
  {
    code: 'CH',
    continent: 'Europe',
    areaInKm2: 41284,
    population: 8476005,
    capital: 'Bern',
    name: {
      English: 'Switzerland',
      Arabic: 'سويسرا',
      Chinese: '瑞士',
      French: 'Suisse',
      Hindi: 'स्विट्जरलैंड',
      Korean: '스위스',
      Japanese: 'スイス',
      Russian: 'Швейцария'
    }
  },
  {
    code: 'SY',
    continent: 'Asia',
    areaInKm2: 185180,
    population: 18269868,
    capital: 'Damascus',
    name: {
      English: 'Syrian',
      Arabic: 'الجمهورية العربية السورية',
      Chinese: '叙利亚',
      French: 'Syrienne',
      Hindi: 'सीरिया अरब गणतंत्र',
      Korean: '시리아',
      Japanese: 'シリアアラブ',
      Russian: 'Сирийская Арабская Республика'
    }
  },
  {
    code: 'TW',
    continent: 'Asia',
    areaInKm2: 36193,
    population: 23626456,
    capital: 'Taipei',
    name: {
      English: 'Taiwan - Province of China',
      Arabic: 'تايوان، مقاطعة الصين ',
      Chinese: '中国台湾省 ',
      French: 'Taiwan - province de Chine',
      Hindi: 'ताइवान - चीन के प्रांत',
      Korean: '대만 - 중국',
      Japanese: '台湾、中国',
      Russian: 'Тайвань - провинция Китая'
    }
  },
  {
    code: 'TJ',
    continent: 'Asia',
    areaInKm2: 143100,
    population: 8921343,
    capital: 'Dushanbe',
    name: {
      English: 'Tajikistan',
      Arabic: 'طاجيكستان',
      Chinese: '塔吉克斯坦',
      French: 'Tadjikistan',
      Hindi: 'तजाकिस्तान',
      Korean: '타지키스탄',
      Japanese: 'タジキスタン',
      Russian: 'Таджикистан'
    }
  },
  {
    code: 'TZ',
    continent: 'Africa',
    areaInKm2: 945087,
    population: 57310019,
    capital: 'Dodoma',
    name: {
      English: 'Tanzania',
      Arabic: 'جمهورية تنزانيا المتحدة',
      Chinese: '坦桑尼亚',
      French: 'Tanzanie',
      Hindi: 'तंजानिया',
      Korean: '탄자니아',
      Japanese: 'タンザニア',
      Russian: 'Танзания'
    }
  },
  {
    code: 'TH',
    continent: 'Asia',
    areaInKm2: 513120,
    population: 69037513,
    capital: 'Bangkok',
    name: {
      English: 'Thailand',
      Arabic: 'تايلاند',
      Chinese: '泰国',
      French: 'Thaïlande',
      Hindi: 'थाईलैंड',
      Korean: '태국',
      Japanese: 'タイ',
      Russian: 'Таиланд'
    }
  },
  {
    code: 'TL',
    continent: 'Asia',
    areaInKm2: 'n/a',
    population: 1296311,
    capital: 'Dili',
    name: {
      English: 'Timor-Leste',
      Arabic: 'تيمور الشرقية',
      Chinese: '东帝汶',
      French: 'Timor-Leste',
      Hindi: 'तिमोर-लेस्ते',
      Korean: '동 티모르',
      Japanese: '東ティモール',
      Russian: 'Восточный Тимор'
    }
  },
  {
    code: 'TG',
    continent: 'Africa',
    areaInKm2: 56785,
    population: 7797694,
    capital: 'Lome',
    name: {
      English: 'Togo',
      Arabic: 'ليذهب',
      Chinese: '多哥',
      French: 'Aller',
      Hindi: 'जाना',
      Korean: '가다',
      Japanese: '行く',
      Russian: 'Идти'
    }
  },
  {
    code: 'TO',
    continent: 'Oceania',
    areaInKm2: 747,
    population: 108020,
    capital: "Nuku'alofa",
    name: {
      English: 'Tonga',
      Arabic: 'تونغا',
      Chinese: '汤加',
      French: 'Tonga',
      Hindi: 'टोंगा',
      Korean: '통가',
      Japanese: 'トンガ',
      Russian: 'Тонга'
    }
  },
  {
    code: 'TT',
    continent: 'Americas',
    areaInKm2: 5130,
    population: 1369125,
    capital: 'Port of Spain',
    name: {
      English: 'Trinidad and Tobago',
      Arabic: 'ترينداد وتوباغو',
      Chinese: '特立尼达和多巴哥',
      French: 'Trinité-et-Tobago',
      Hindi: 'त्रिनिदाद और टोबैगो',
      Korean: '트리니다드 토바고',
      Japanese: 'トリニダード・トバゴ',
      Russian: 'Тринидад и Тобаго'
    }
  },
  {
    code: 'TN',
    continent: 'Africa',
    areaInKm2: 163610,
    population: 11532127,
    capital: 'Tunis',
    name: {
      English: 'Tunisia',
      Arabic: 'تونس',
      Chinese: '突尼斯',
      French: 'Tunisie',
      Hindi: 'ट्यूनीशिया',
      Korean: '튀니지',
      Japanese: 'チュニジア',
      Russian: 'Тунис'
    }
  },
  {
    code: 'TR',
    continent: 'Asia',
    areaInKm2: 783562,
    population: 80745020,
    capital: 'Ankara',
    name: {
      English: 'Turkey',
      Arabic: 'Turkey',
      Chinese: '土耳其',
      French: 'La Turquie',
      Hindi: 'Turkey',
      Korean: 'Turkey',
      Japanese: 'トルコ共和国',
      Russian: 'Turkey'
    }
  },
  {
    code: 'TM',
    continent: 'Asia',
    areaInKm2: 488100,
    population: 5758075,
    capital: 'Aşgabat',
    name: {
      English: 'Turkmenistan',
      Arabic: 'تركمانستان',
      Chinese: '土库曼斯坦',
      French: 'Turkménistan',
      Hindi: 'तुर्कमेनिस्तान',
      Korean: '투르크 메니스탄',
      Japanese: 'トルクメニスタン',
      Russian: 'Туркменистан'
    }
  },
  {
    code: 'TC',
    continent: 'Americas',
    areaInKm2: 948,
    population: 35446,
    capital: 'Cockburn Town',
    name: {
      English: 'Turks and Caicos Islands',
      Arabic: 'جزر تركس وكايكوس',
      Chinese: '特克斯和凯科斯群岛',
      French: 'îles Turques-et-Caïques',
      Hindi: 'तुर्क्स और कैकोज़ द्वीपसमूह',
      Korean: '터크 스 케이 커스 제도',
      Japanese: 'タークスカイコス諸島',
      Russian: 'Острова Теркс и Кайкос'
    }
  },
  {
    code: 'TV',
    continent: 'Oceania',
    areaInKm2: 26,
    population: 11192,
    capital: 'Funafuti',
    name: {
      English: 'Tuvalu',
      Arabic: 'توفالو',
      Chinese: '图瓦卢',
      French: 'Tuvalu',
      Hindi: 'तुवालु',
      Korean: '투발루',
      Japanese: 'ツバル',
      Russian: 'Тувалу'
    }
  },
  {
    code: 'UG',
    continent: 'Africa',
    areaInKm2: 241550,
    population: 42862958,
    capital: 'Kampala',
    name: {
      English: 'Uganda',
      Arabic: 'أوغندا',
      Chinese: '乌干达',
      French: 'Ouganda',
      Hindi: 'युगांडा',
      Korean: '우간다',
      Japanese: 'ウガンダ',
      Russian: 'Уганда'
    }
  },
  {
    code: 'UA',
    continent: 'Europe',
    areaInKm2: 603500,
    population: 44222947,
    capital: 'Kiev',
    name: {
      English: 'Ukraine',
      Arabic: 'أوكرانيا',
      Chinese: '乌克兰',
      French: 'Ukraine',
      Hindi: 'यूक्रेन',
      Korean: '우크라이나',
      Japanese: 'ウクライナ',
      Russian: 'Украина'
    }
  },
  {
    code: 'AE',
    continent: 'Asia',
    areaInKm2: 83600,
    population: 9400145,
    capital: 'Abu Dhabi',
    name: {
      English: 'United Arab Emirates',
      Arabic: 'الإمارات العربية المتحدة',
      Chinese: '阿联酋',
      French: 'Emirats Arabes Unis',
      Hindi: 'संयुक्त अरब अमीरात',
      Korean: '아랍 에미리트',
      Japanese: 'アラブ首長国連邦',
      Russian: 'Объединенные Арабские Эмираты'
    }
  },
  {
    code: 'GB',
    continent: 'Europe',
    areaInKm2: 242495,
    population: 66181585,
    capital: 'London',
    name: {
      English: 'United Kingdom',
      Arabic: 'المملكة المتحدة لبريطانيا العظمى وايرلندا الشمالية',
      Chinese: '英国',
      French: "Royaume-Uni de Grande-Bretagne et d'Irlande du Nord",
      Hindi: 'ग्रेट ब्रिटेन और उत्तरी आयरलैंड के यूनाइटेड किंगडम',
      Korean: '영국 대 브리튼 및 북 아일랜드',
      Japanese: 'イギリスと北アイルランド',
      Russian: 'Соединенное Королевство Великобритании и Северной Ирландии'
    }
  },
  {
    code: 'US',
    continent: 'Americas',
    areaInKm2: 9525067,
    population: 324459463,
    capital: 'Washington DC',
    name: {
      English: 'United States of America',
      Arabic: 'الولايات المتحدة',
      Chinese: '美国',
      French: "les États-Unis d'Amérique",
      Hindi: 'संयुक्त राज्य अमरीका',
      Korean: '미국',
      Japanese: 'アメリカ合衆国',
      Russian: 'Соединенные Штаты Америки'
    }
  },
  {
    code: 'UM',
    continent: 'Americas',
    areaInKm2: 6959.41,
    population: 104901,
    capital: '',
    name: {
      English: 'United States Minor Outlying Islands',
      Arabic: 'جزر الولايات المتحدة البعيدة الصغرى',
      Chinese: '美国小离岛',
      French: 'Îles mineures éloignées des États-Unis',
      Hindi: 'संयुक्त राज्य अमेरिका के छोटे दूरस्थ द्वीपसमूह',
      Korean: '미국령 군소 제도',
      Japanese: 'アメリカ合衆国外諸島',
      Russian: 'Малые отдаленные острова Соединенных Штатов'
    }
  },
  {
    code: 'UY',
    continent: 'Americas',
    areaInKm2: 176215,
    population: 3456750,
    capital: 'Montevideo',
    name: {
      English: 'Uruguay',
      Arabic: 'أوروغواي',
      Chinese: '乌拉圭',
      French: 'Uruguay',
      Hindi: 'उरुग्वे',
      Korean: '우루과이',
      Japanese: 'ウルグアイ',
      Russian: 'Уругвай'
    }
  },
  {
    code: 'UZ',
    continent: 'Asia',
    areaInKm2: 447400,
    population: 31910641,
    capital: 'Tashkent',
    name: {
      English: 'Uzbekistan',
      Arabic: 'أوزبكستان',
      Chinese: '乌兹别克斯坦',
      French: 'Ouzbékistan',
      Hindi: 'उज़्बेकिस्तान',
      Korean: '우즈베키스탄',
      Japanese: 'ウズベキスタン',
      Russian: 'Узбекистан'
    }
  },
  {
    code: 'VU',
    continent: 'Oceania',
    areaInKm2: 12189,
    population: 276244,
    capital: 'Port Vila',
    name: {
      English: 'Vanuatu',
      Arabic: 'فانواتو',
      Chinese: '瓦努阿图',
      French: 'Vanuatu',
      Hindi: 'वानुअतु',
      Korean: '바누아투',
      Japanese: 'バヌアツ',
      Russian: 'Вануату'
    }
  },
  {
    code: 'VE',
    continent: 'Americas',
    areaInKm2: 916445,
    population: 31977065,
    capital: 'Caracas',
    name: {
      English: 'Venezuela',
      Arabic: 'فنزويلا',
      Chinese: '委内瑞拉',
      French: 'Venezuela',
      Hindi: 'वेनेजुएला',
      Korean: '베네수엘라',
      Japanese: 'ベネズエラ',
      Russian: 'Венесуэла'
    }
  },
  {
    code: 'VN',
    continent: 'Asia',
    areaInKm2: 331212,
    population: 95540800,
    capital: 'Hanoi',
    name: {
      English: 'Viet Nam',
      Arabic: 'فييت نام',
      Chinese: '越南',
      French: 'Viet Nam',
      Hindi: 'वियतनाम',
      Korean: '베트남',
      Japanese: 'ベトナム',
      Russian: 'Вьетнам'
    }
  },
  {
    code: 'VG',
    continent: '',
    areaInKm2: 'n/a',
    population: 'n/a',
    capital: 'Road Town',
    name: {
      English: 'Virgin Islands (British)',
      Arabic: 'جزر فيرجين (البريطانية)',
      Chinese: '维尔京群岛（英属）',
      French: 'Îles Vierges (britanniques)',
      Hindi: 'वर्जिन आइलैंड्स (ब्रिटिश)',
      Korean: '버진 아일랜드 (영국령)',
      Japanese: 'バージン諸島（英国）',
      Russian: 'Виргинские острова (англ.)'
    }
  },
  {
    code: 'VI',
    continent: '',
    areaInKm2: 'n/a',
    population: 'n/a',
    capital: 'Charlotte Amalie',
    name: {
      English: 'Virgin Islands (U.S.)',
      Arabic: 'جزر فيرجن (الولايات المتحدة)',
      Chinese: '维尔京群岛（美国）',
      French: 'Îles Vierges (États-Unis)',
      Hindi: 'वर्जिन द्वीप समूह (यूएस)',
      Korean: '버진 제도 (미국)',
      Japanese: 'バージン諸島（米国）',
      Russian: 'Виргинские острова (США)'
    }
  },
  {
    code: 'YE',
    continent: 'Asia',
    areaInKm2: 527968,
    population: 28250420,
    capital: "Sana'a",
    name: {
      English: 'Yemen',
      Arabic: 'اليمن',
      Chinese: '也门',
      French: 'Yémen',
      Hindi: 'यमन',
      Korean: '예멘 아랍 공화국',
      Japanese: 'イエメン',
      Russian: 'Йемен'
    }
  },
  {
    code: 'ZM',
    continent: 'Africa',
    areaInKm2: 752612,
    population: 17094130,
    capital: 'Lusaka',
    name: {
      English: 'Zambia',
      Arabic: 'زامبيا',
      Chinese: '赞比亚',
      French: 'Zambie',
      Hindi: 'जाम्बिया',
      Korean: '잠비아',
      Japanese: 'ザンビア',
      Russian: 'Замбия'
    }
  },
  {
    code: 'ZW',
    continent: 'Africa',
    areaInKm2: 390757,
    population: 16529904,
    capital: 'Harare',
    name: {
      English: 'Zimbabwe',
      Arabic: 'زيمبابوي',
      Chinese: '津巴布韦',
      French: 'Zimbabwe',
      Hindi: 'जिम्बाब्वे',
      Korean: '짐바브웨',
      Japanese: 'ジンバブエ',
      Russian: 'Зимбабве'
    }
  },
  {
    code: '',
    continent: '—',
    areaInKm2: 510072000,
    population: 7550262101,
    capital: '',
    name: {
      English: 'World',
      Arabic: 'World',
      Chinese: '世界',
      French: 'World',
      Hindi: 'World',
      Korean: 'World',
      Japanese: 'World',
      Russian: 'World'
    }
  }
];
